...
3. **Polyglot Reachability Engine (`src/reachability/`):** Cross-references standard SCA vulnerabilities with AST/Regex source code analysis. It natively supports Node, Python, Java, C#, and Rust to flag whether a vulnerable dependency is actually utilized in the application tree, drastically cutting false positives.
